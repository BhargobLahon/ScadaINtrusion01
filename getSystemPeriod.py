...
def trainGMMs(trainSamples,sysPeriod=1,nMaxMixtures=10,cvType='full') :
	...
	max_BIC_List = []
	for i in range(1,nMaxMixtures+1):
		max_BIC = -np.infty
		for j in range(0,nStages) :
...
if __name__ == "__main__" :
	...
	print(f"nSamples = {nSamples} n Signals = {len(timeSeries[0])}")
	...
	if testPCA == True :
		...
		print(f"PCA 99 percentile optDimensionality = {optDimensionality}")
	...
	nLags = 50
	for i in range(0,nSignals) :
		print(f"Extracting candidate periods for signal {i} ...")
		candidatePeriods,pThreshold = getPeriodHints(X=np.array(map(itemgetter(i),timeSeries)),fs=1.0)
		print(f"Candidate Periods = {candidatePeriods}")
		print(f"pThreshold = {pThreshold}")

		print(f"Estimating Acf for signal {i} ...")
	...
	while start < nMaxPeriods + 1:
		...
		print(f"Training HMM for SysPeriod = {sysPeriod}")
